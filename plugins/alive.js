...
module.exports = {
    ...
    run: async (sock, message, args, { sender, contextInfo }) => {
        ...
        const theme  = (config.THEME || 'Sachith').toLowerCase();

        const now    = moment().tz(TZ);
        const date   = now.format('ddd, D MMM YYYY');
        const time   = now.format('hh:mm A');

        const botName  = t.botName  || '⛁ CYBER PANDA MD ⛁';
        const footer   = t.footer   || '⛁ CYBER PANDA MD ⛁';
        const alive    = t.alive    || 'System online. All functions nominal.';
        const emoji    = t.aliveEmoji || '🐼';
        const imageUrl = config.ALIVE_IMG || t.pic1 || 'https://i.ibb.co/xqd5hcNw/Dark-E-sports-Illustrative-Panda-Face-Gaming-Logo.jpg';

        const modeEmoji = mode === 'PUBLIC' ? '💗' : mode === 'PRIVATE' ? '🔒' : '❤️';
