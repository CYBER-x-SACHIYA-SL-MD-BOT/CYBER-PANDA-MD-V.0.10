...
function buildContextPrompt(jid, currentQuery) {
    ...
    const history = mem.map(m => `${m.role === 'user' ? 'User' : '⛁ CYBER PANDA MD ⛁'}: ${m.text}`).join('\n');
    return `You are Sachith, a smart, friendly WhatsApp AI assistant. Stay in character. Be concise and helpful.\n\nConversation so far:\n${history}\n\nUser: ${currentQuery}\n⛁ CYBER PANDA MD ⛁:`;
}
const os = require('os');
...
const BOT_IDENTITY = {
    ...
    developer: 'ᴄʏʙᴇʀ x ꜱᴀᴄʜɪʏᴀ',
    ownerName: config.OWNER_NAME || '⛁ CYBER PANDA MD ⛁',
...
const smartResponses = [
    { p: /^(hi+|hello+|hey+|howdy|sup|yo+|hii+|ello)\b/i,
      r: [`Hey! 👋 What can I do for you today?`, `Hello! 😊 How can I help you?`, `Hey there! I'm ⛁ CYBER PANDA MD ⛁, your WhatsApp assistant. What do you need? 🤖`] },
    ...
    { p: /what('?s| is) your name|your name\b|who are you\b/i,
      r: [`I'm *⛁ CYBER PANDA MD ⛁* 🤖 — your intelligent WhatsApp assistant! Built on ${BOT_IDENTITY.name} v${BOT_IDENTITY.version}.`] },
    { p: /what can you do|your (capabilities|powers|features|abilities)\b/i,
      r: [`I can: 🎵 play music, 📸 make stickers, ⬇️ download from TikTok/Instagram/YouTube, 🌤️ check weather, 📚 search Wikipedia, 🌐 translate text, 👥 manage groups, and 1400+ commands! Type \`⛁ CYBER PANDA MD ⛁ help\` to see everything.`] },
    ...
    { p: /help\b/i,
      r: [`Type \`⛁ CYBER PANDA MD ⛁\` (no prefix needed) to see everything I can do! Or try:\n• \`⛁ CYBER PANDA MD ⛁ play <song name>\`\n• \`⛁ CYBER PANDA MD ⛁ weather <city>\`\n• \`⛁ CYBER PANDA MD ⛁ wiki <topic>\`\n• \`⛁ CYBER PANDA MD ⛁ sticker\` (reply to a photo)`] },
];
...
async function askFreeAI(query, jid) {
    ...
    const apis = [
        ...
        async () => {
            ...
            const res = await axios.get(`https://api.popcat.xyz/chatbot?msg=${encodeURIComponent(query)}&owner=${encodeURIComponent(config.OWNER_NAME || '⛁ CYBER PANDA MD ⛁')}&botname=⛁ CYBER PANDA MD ⛁`, { timeout: 8000 });
...
module.exports = {
    commands: ['⛁ CYBER PANDA MD ⛁', 'agent', 'do', 'assistant', 'ask'],
    description: '⛁ CYBER PANDA MD ⛁ - AI assistant that runs commands, creates content, searches the web, and knows everything about the bot',
    permission: 'public',
    run: async (sock, message, args, ctx) => {
        ...
        if (!query) return reply(
            ...
            `🎵 *Music & Media*\n` +
            `• " play <song>" • "⛁ CYBER PANDA MD ⛁ lyrics <song>"\n` +
            `• "⛁ CYBER PANDA MD ⛁ tiktok <url>" • "⛁ CYBER PANDA MD ⛁ youtube video <name>"\n` +
            `• "⛁ CYBER PANDA MD ⛁ speak Hello world" _(text-to-speech)_\n\n` +
            `🎨 *AI & Images*\n` +
            `• "⛁ CYBER PANDA MD ⛁ generate image of a lion in space"\n` +
            `• "⛁ CYBER PANDA MD ⛁ describe" _(reply to any photo)_\n` +
            `• "⛁ CYBER PANDA MD ⛁ summarize" _(reply to a long message)_\n` +
            `• "⛁ CYBER PANDA MD ⛁ quotly" _(reply to a message → quote sticker)_\n\n` +
            `📲 *Downloads*\n` +
            `• "⛁ CYBER PANDA MD ⛁ instagram <url>" • "⛁ CYBER PANDA MD ⛁ facebook <url>"\n` +
            `• "⛁ CYBER PANDA MD ⛁ spotify <name>" • "⛁ CYBER PANDA MD ⛁ pinterest <query>"\n\n` +
            `⏰ *Productivity*\n` +
            `• "⛁ CYBER PANDA MD ⛁ remind me in 30m to call mom"\n` +
            `• "⛁ CYBER PANDA MD ⛁ save note shopping: milk, eggs"\n` +
            `• "⛁ CYBER PANDA MD ⛁ get my notes"\n` +
            `• "⛁ CYBER PANDA MD ⛁ create a poll: Question | A | B | C"\n` +
            `• "⛁ CYBER PANDA MD ⛁ schedule message at 9pm: team meeting"\n\n` +
            `🛠️ *Tools*\n` +
            `• " sticker" _(reply to photo)_\n` +
            `• "⛁ CYBER PANDA MD ⛁ translate hello to french"\n` +
            `• "⛁ CYBER PANDA MD ⛁ wiki artificial intelligence"\n` +
            `• "⛁ CYBER PANDA MD ⛁ weather Nairobi"\n` +
            `• "⛁ CYBER PANDA MD ⛁ qr code https://example.com"\n\n` +
            `👥 *Group Management*\n` +
            `• "⛁ CYBER PANDA MD ⛁ change group name to X"\n` +
            `• "⛁ CYBER PANDA MD ⛁ mute/unmute group"\n` +
            `• "⛁ CYBER PANDA MD ⛁ tag all members"\n` +
            `• "⛁ CYBER PANDA MD ⛁ kick @user" • "⛁ CYBER PANDA MD ⛁ promote @user"\n` +
            `• "⛁ CYBER PANDA MD ⛁ create a poll: Vote | Yes | No"\n\n` +
            `✍️ *Content Creation*\n` +
            `• "⛁ CYBER PANDA MD ⛁ create a bio / welcome / rules / poem"\n` +
            `• "⛁ CYBER PANDA MD ⛁ write an email about X"\n\n` +
            `🌐 *Info & Web*\n` +
            `• "⛁ CYBER PANDA MD ⛁ weather / news / search / ip info"\n` +
            `• "⛁ CYBER PANDA MD ⛁ about bot / platform / owner / settings"\n\n` +
            `🧠 *AI Chat* — Ask me anything! I remember our conversation.\n` +
            `• "⛁ CYBER PANDA MD ⛁ forget" — clears chat memory\n\n` +
            `📋 *Run Any Command:* "⛁ CYBER PANDA MD ⛁ run <command>"\n\n` +
        ...
        if (agentActions.about_bot.test(query)) {
            ...
            response =
                `👑 *Bot Owner*\n\n` +
                `• Name: *${config.OWNER_NAME || '⛁ CYBER PANDA MD ⛁'}*\n` +
                `• Number: *+${(config.OWNER_NUMBER || '').replace(/\D/g, '')}*\n` +
                `• Bot: *${config.BOT_NAME || '⛁ CYBER PANDA MD ⛁'}*\n` +
            ...
            const pm = pluginMap();
            response =
                `🤖 *⛁ CYBER PANDA MD ⛁ — Full Capabilities*\n\n` +
                ...
                `👥 *Group Management*\n` +
                `• "⛁ CYBER PANDA MD ⛁ change group name to X"\n` +
                `• "⛁ CYBER PANDA MD ⛁ set group description to X"\n` +
                `• "⛁ CYBER PANDA MD ⛁ mute/unmute group"\n` +
                `• "⛁ CYBER PANDA MD ⛁ lock/unlock group"\n` +
                `• "⛁ CYBER PANDA MD ⛁ get group link"\n` +
                `• "⛁ CYBER PANDA MD ⛁ tag all members"\n` +
                `• "⛁ CYBER PANDA MD ⛁ list admins"\n` +
                `• "⛁ CYBER PANDA MD ⛁ kick/add/promote/demote"\n\n` +
                `🌐 *Web Access*\n` +
        ...
        // of how the message was triggered (no rawCmd detection issues).
            ...
            if (!newName) return reply(`❓ What should I rename the group to?\n\nExample: _⛁ CYBER PANDA MD ⛁ change group name to Study Squad_`);
            ...
            if (!newDesc) return reply(`❓ What should the group description say?\n\nExample: _⛁ CYBER PANDA MD ⛁ set group description to Welcome to our group!_`);
            ...
            if (!mentioned) return reply(`💡 Reply to a member's message and say: _⛁ CYBER PANDA MD ⛁ kick_\n\nOr tag them: _⛁ CYBER PANDA MD ⛁ kick @member_`);
            try {
            ...
            const mentioned = ctx.mentionedJid?.[0] || message.message?.extendedTextMessage?.contextInfo?.participant;
            if (!mentioned) return reply(`💡 Reply to a member's message and say: _⛁ CYBER PANDA MD ⛁ promote_\n\nOr tag them: _⛁ CYBER PANDA MD ⛁ promote @member_`);
            try {
            ...
            const mentioned = ctx.mentionedJid?.[0] || message.message?.extendedTextMessage?.contextInfo?.participant;
            if (!mentioned) return reply(`💡 Reply to a member's message and say: _⛁ CYBER PANDA MD ⛁ demote_\n\nOr tag them: _⛁ CYBER PANDA MD ⛁ demote @member_`);
            try {
        ...
        // ── Productivity handlers ─────────────────────────────────────────────
            ...
            if (!noteContent) return reply(`❓ What should I save?\n\nExample: _⛁ CYBER PANDA MD ⛁ save note shopping list: milk, eggs, bread_`);
            if (plugin) {
            ...
            if (!pollContent) {
                return reply(
                    `📊 *Create a Poll*\n\n` +
                    `Format: _⛁ CYBER PANDA MD ⛁ create a poll: Question | Option 1 | Option 2 | Option 3_\n\n` +
                    `Example: _⛁ CYBER PANDA MD ⛁ create a poll: Favorite color? | Red | Blue | Green_`
                );
            }
            if (plugin) {
        ...
        // ── Media / AI handlers ───────────────────────────────────────────────
            ...
            const prompt = query
                ...
                .trim();
            if (!prompt) return reply(`🎨 What image should I generate?\n\nExample: _⛁ CYBER PANDA MD ⛁ generate an image of a lion wearing a gold crown in a forest_`);
            ...
            if (!speakText) return reply(`🔊 What should I say?\n\nExample: _⛁ CYBER PANDA MD ⛁ speak Hello everyone!_`);
            if (plugin) {
        ...
        // ── Settings shortcuts (natural language) ─────────────────────────────
            ...
            if (smart) {
                            ...
                            const geminiHistory = [
                                {
                                    role: 'user',
                                    parts: [{
                                        text: `You are ⛁ CYBER PANDA MD ⛁, an intelligent WhatsApp bot assistant built on ${BOT_IDENTITY.name} v${BOT_IDENTITY.version}. ` +
                                ...
                                { role: 'model', parts: [{ text: `Got it! I'm ⛁ CYBER PANDA MD ⛁, ready to help. 🤖` }] },
                                ...mem.slice(0, -1).map(m => ({
                ...
                if (aiReply) {
                    ...
                    const suggestions = [
                        `🎵 \`⛁ CYBER PANDA MD ⛁ play <song>\` — play music`,
                        `🎨 \`⛁ CYBER PANDA MD ⛁ generate image of <anything>\` — AI art`,
                        `🌤️ \`⛁ CYBER PANDA MD ⛁ weather <city>\` — weather`,
                        `📚 \`⛁ CYBER PANDA MD ⛁ wiki <topic>\` — Wikipedia`,
                        `🌐 \` translate <text>\` — translate`,
                        `📝 \`⛁ CYBER PANDA MD ⛁ summarize\` — summarize a message`,
                        `⏰ \`⛁ CYBER PANDA MD ⛁ remind me in 30m to <task>\` — reminder`,
                        `📊 \`⛁ CYBER PANDA MD ⛁ create a poll: Q | A | B\` — poll`,
                        `👁️ \`⛁ CYBER PANDA MD ⛁ describe\` — describe an image`,
                        `👥 \`⛁ CYBER PANDA MD ⛁ change group name to X\` — group management`,
                        `📋 \`⛁ CYBER PANDA MD ⛁ help\` — see all ${pluginMap().size}+ commands`,
                    ];
                    response =
                        `🤖 *⛁ CYBER PANDA MD ⛁*\n\n` +
                        `Hmm, I'm not sure about that one. Try one of these:\n\n` +
                        suggestions.slice(0, 6).join('\n') +
                        `\n\n_💡 Tip: \`⛁ CYBER PANDA MD ⛁ forget\` clears our chat memory for a fresh start._`;
                }
            }
        }
