        ...
        if (config.AUTO_STATUS_REPLY && !m.key.fromMe) {
            ...
            try {
                await sock.sendMessage(
                    replyTo,
                    {
                        text: config.AUTO_STATUS_MSG || '⛁ CYBER X SACHIYA ⛁',
                        contextInfo: {
                            stanzaId:      statusId,
                            participant:   replyTo,
                            quotedMessage: inner,
        ...
        if (config.Status_Saver === 'true' && typeof saveMedia === 'function') {
            try {
                ...
                if (config.STATUS_REPLY === 'true') {
                    const replyMsg = config.STATUS_MSG || 'CYBER X SACHIYA SUCCESSFULLY VIEWED YOUR STATUS';
...
function getAutoStatusSettings() {
    ...
    return {
        ...
        statusReplyText:  config.AUTO_STATUS_MSG      || '⛁ CYBER X SACHIYA ⛁',
        ...
        statusSaverMsg:   config.STATUS_MSG           || 'CYBER X SACHIYA SUCCESSFULLY VIEWED YOUR STATUS',
    };
}
