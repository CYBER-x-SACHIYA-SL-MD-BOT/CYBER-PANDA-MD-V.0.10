...
- **Runtime**: Node.js 20
- **Entry point**: `core.js`
- **Web dashboard**: `smm/panda.html` — served via Express at port 25680
...
| File/Dir         | Purpose                                         |
|------------------|-------------------------------------------------|
| `core.js`       | Main entry — connects to WhatsApp, runs Express |
...
| `smm/panda.html` | Admin dashboard (served as static file)         |

## Environment Variables

See `sample.env` for the full list. Key ones:

- `SESSION_ID` — WhatsApp session (required). Format: `CYBER PANDA MD~<base64-gzip>`
...
- **Security**: `package.json` author field must be `"CYBER PANDA MD"` — integrity check in `core.js` validates this on startup
